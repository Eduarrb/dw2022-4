<?php
        ...
        if(isset($_POST['registrar'])){
            ...
            if(!empty($errores)){
                ...
                if(registro_usuario($user_nombres, $user_apellidos, $user_email, $user_pass)){
                    set_mensaje(display_msj("Registro satisfactorio. Por favor revisa tu correo o spam para la activacion de tu cuenta. Esto puede tardar unos minutos", "success"));
    ...
    function registro_usuario($nombres, $apellidos, $email, $pass){
        ...
        confirm($query);
        $mensaje = "Por favor activa tu cuenta mediante este <a href='http://localhost/dw2022-4/04%20CMS/public/activate.php?email={$user_email}&token={$user_token}' target='_blank'>LINK</a>";
